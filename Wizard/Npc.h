#ifndef NPC_H
#define NPC_H

#include <bits/stdc++.h>
#include "Animation.h"
#include "Game.h"
#include "Entity.h"
class NPC: public Entity{
protected:
    double direction_x = 0;
    double direction_y = 0;
    int direct = 0;
    int maxAttackFrame;
    int maxRunFrame;
    int maxIdleFrame;
private:
    double HP;
    double maxHP;
    double speed;
    AnimationObject* anima;
    bool Run = true;
    bool Attack = true;
    int attackFrame = 0;
public:
    void setAnima(AnimationObject* animation){
        anima = animation;
    }
    void  setAttackFrame(int i){
        attackFrame = i;
    }
    int getAttackFrame(){
        return attackFrame;
    }
    AnimationObject* getAnima(){
        return anima;
    }
    void setHP(double HP){
        this->HP = HP;
    }
    void setMaxHP(double maxHP){
        this->maxHP = maxHP;
    }
    void setSpeed(double speed){
        this->speed = speed;
    }
    double getHP(){
        return this->HP;
    }
    double getMaxHP(){
        return this->maxHP;
    }
    double getSpeed(){
        return this->speed;
    }
    bool canRun(){
        return Run;
    }
    bool canAttack(){
        return Attack;
    }
    void setRun(bool a){
        Run = a;
    }
    void setAttack(bool a){
        Attack = a;
    }
    void nextAttackFrame(){
        attackFrame++;
    }
};

class Spike: public NPC{
private:
    double toX;
    double toY;
    string name;
    int timeBorn = 0;
public:
    Spike(){}
    Spike(string name, double hp, double maxHP, double speed, double scale, double x, double y, AnimationObject* animation, SDL_Renderer* renderer);
    void setToXY(double toX, double toY){
        this->toX = toX;
        this->toY = toY;
    }
    void setToX(double toX){
        this->toX = toX;
    }
    void setToY(double toY){
        this->toY = toY;
    }
    double getToX() {return toX;}
    double getToY() {return toY;}

    void setName(string name){
        this->name = name;
    }
    string getName(){
        return this->name;
    }
    double getDirectXY(){
        return min(1.0, getSpeed()/sqrt((toX-getX())*(toX-getX()) + (toY-getY())*(toY-getY())));
    }
    void update();
    void renderNPC(double x, double y){
        this->renderCenter_Cam( x,  y);
    }
};
class Bullet: public NPC{
private:
    int id;
    double time;
    int speedOfFrame;
    int numberOfFrame;
    int frame;
    int timeBorn;
    double toX;
    double toY;
    bool loopAnimation;
public:
    Bullet(){}
    Bullet(string name, double x, double y, double toX, double toY, double scale, double speed, int speedOfFrame, bool loopAnimation, SDL_Renderer* renderer, AnimationObject* animation);
    //renew frame
    void update(double timeAdd);
    double getDirectXY(){
        return min(1.0, getSpeed()/sqrt((toX-getX())*(toX-getX()) + (toY-getY())*(toY-getY())));
    }
    void renderNPC(double x, double y);

    void setToXY(double toX, double toY){
        this->toX = toX;
        this->toY = toY;
    }
    void setToX(double toX){
        this->toX = toX;
    }
    void setToY(double toY){
        this->toY = toY;
    }
    int    getFrame()   {return this->frame;}
    double getSpeedOfFrame()   {return this->speedOfFrame;}
    double getNumberOfFrame() {return this->numberOfFrame;}
    double getTime()    {return this->time;}
    int    getID()      {return this->id;}
};
class Player: public NPC{
private:
    Game* game;
public:
    Player();
    Player(double hp, double maxHP, double speed, double scale, AnimationObject* animation, SDL_Renderer* renderer, Game* game);
    void playAttackAnimation();
    void updateDirect(double x, double y);
    void update();
    void renderNPC(double x, double y);
};

class Enemy: public NPC{
private:
    double toX;
    double toY;
    string name;
public:
    Enemy(){}
    Enemy(string name, double hp, double maxHP, double speed, double scale, double x, double y, AnimationObject* animation, SDL_Renderer* renderer);
    void setToXY(double toX, double toY){
        this->toX = toX;
        this->toY = toY;
    }
    void setToX(double toX){
        this->toX = toX;
    }
    void setToY(double toY){
        this->toY = toY;
    }
    double getToX() {return toX;}
    double getToY() {return toY;}

    void setName(string name){
        this->name = name;
    }
    string getName(){
        return this->name;
    }
    double getDirectXY(){
        return min(1.0, getSpeed()/sqrt((toX-getX())*(toX-getX()) + (toY-getY())*(toY-getY())));
    }
    void update();
    void renderNPC(double x, double y);
};
#endif // NPC_H
